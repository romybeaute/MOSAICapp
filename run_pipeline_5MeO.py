"""
5MeO pipeline — reuses cached Qwen3-4B embeddings, applies UMAP/HDBSCAN params
chosen by Optuna, then LLM-labels topics and saves plots.

Usage:
    sbatch run_pipeline_5MeO.sh                       # Optuna trial 0 → 55 topics
    python run_pipeline_5MeO.py --mcs 29 --ms 24 --nn 24 --nc 8 --tag t0
"""
...
parser.add_argument("--tag",        type=str, required=True, help="Short label for outputs (e.g. t0)")
...
log_file = f"pipeline_5MeO_{args.tag}.log"
logging.basicConfig(
    level=logging.INFO,
...
DATASET_NAME    = "5MeO"
CSV_PATH        = "data/5-Me0-DMT/5MeO_naturalistic_preprocessed.csv"
...
# ── Paths — reuse shared 5MeO embeddings cache ────────────────────────────────
...
# Variant-specific paths (keyed by tag)
...
if not Path(DOCS_FILE).exists() or not Path(EMBEDDINGS_FILE).exists():
    raise FileNotFoundError(
        "Embeddings not found — run run_embed_5MeO.py first to generate them."
    )
...
if args.reduce_outliers:
    ...
    if not (Path(_topics_path).exists() and Path(_reduced_path).exists()):
        raise FileNotFoundError(f"topics_{_load_tag}.json or reduced_2d_{_load_tag}.npy not found.")
    ...
    # Load existing LLM labels from cache
    LLM_MODEL = args.llm_model
    cached_labels_files = sorted(CACHE_DIR.glob("llm_labels_*.json"))
    labels = {}
    if cached_labels_files:
        labels = {int(k): str(v) for k, v in json.loads(cached_labels_files[-1].read_text()).items()}
        log.info(f"Loaded {len(labels)} cached labels from {cached_labels_files[-1].name}")
    else:
        log.warning("No cached LLM labels found")

    # Reconstruct topic_info with updated counts
    from collections import Counter
    new_counts  = Counter(t for t in topics if t != -1)
    base_csv    = PROJECT_ROOT / f"outputs_5MeO_{_load_tag}" / "topic_info.csv"
    if base_csv.exists():
        topic_info = _pd.read_csv(base_csv)
    ...
    name_map = dict(zip(topic_info["Topic"].astype(int),
                        topic_info["Name"].astype(str) if "Name" in topic_info.columns
                        else topic_info["LLM_Label"].astype(str)))
...
PLOTS_DIR = PROJECT_ROOT / f"outputs_5MeO_{args.tag}"
...
try:
    ...
    fig.suptitle(f"5MeO [{args.tag}]: MOSAIC Topic Map", fontsize=16, y=0.99)
...
ax.set_title(f"5MeO [{args.tag}]: Topic sizes ({n_topics} topics, {100*n_outliers/len(topics):.1f}% outliers)")
plt.tight_layout()
...
try:
    ...
    doc_fig.update_layout(
        title=dict(text=f"<b>5MeO [{args.tag}]: Documents and Topics</b>", x=0.5, xanchor="center"),
